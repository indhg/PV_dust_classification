...
def plot_training_history(history, save_dir='checkpoints'):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('训练 & 验证 损失曲线 (配置B: lr=0.0001+Adam)')
    plt.legend()
    plt.grid(True)
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.title('训练 & 验证 准确率曲线 (配置B: lr=0.0001+Adam)')
    plt.legend()
    plt.grid(True)

    plt.tight_layout()
    save_path = os.path.join(save_dir, 'training_curves_config_b.png')
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.show()
    print(f"训练曲线图已保存至 {save_path}")


def train(model, train_loader, val_loader, epochs=20, lr=0.0001, device='cuda'):
    """
    训练模型（配置B: lr=0.0001 + Adam）
    参数:
        model: 待训练模型
    ...
    for epoch in range(epochs):
        ...
        # 打印进度
        print(f"[配置B] Epoch {epoch+1:2d}/{epochs} | "
